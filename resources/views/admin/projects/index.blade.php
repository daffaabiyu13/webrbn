...
<div class="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
    <form method="GET" action="{{ route('admin.projects.index') }}" class="flex flex-1 flex-wrap gap-2">
        <input type="search" name="search" value="{{ request('search') }}" placeholder="Cari project..."
        ...
        <table class="min-w-full divide-y divide-gray-100">
            <thead class="bg-cream/60 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">
                <tr>
                    ...
                    <th class="px-5 py-3">Client / Lokasi</th>
            ...
            <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-cream/40">
                        <td class="px-5 py-3">
                            <div class="flex items-center gap-3">
                                <img src="{{ $project->imageUrl() }}" alt="" class="h-10 w-14 flex-none rounded-md object-cover ring-1 ring-gray-200">
                                <div class="min-w-0">
                                    <p class="font-medium text-gray-800 truncate">{{ $project->title }}</p>
                                    <p class="text-xs text-gray-500 truncate">{{ $project->slug }}</p>
                                </div>
                            </div>
                        </td>
                        ...
                        <td class="px-5 py-3 text-center text-sm text-gray-600">{{ $project->year ?? '—' }}</td>
                        ...
                        <td class="px-5 py-3 text-right">
                            <div class="inline-flex items-center gap-2">
                                <a href="{{ route('admin.projects.edit', $project) }}" class="rounded-md bg-primary/10 px-3 py-1.5 text-xs font-semibold text-primary hover:bg-primary hover:text-white">Edit</a>
                                <form action="{{ route('admin.projects.destroy', $project) }}" method="POST" onsubmit="return confirm('Hapus project {{ $project->title }}?')">
                                    @csrf @method('DELETE')
                ...
                @empty
                    <tr><td colspan="5" class="px-5 py-12 text-center text-gray-500">Belum ada project. Klik "Tambah Project" untuk mulai.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
