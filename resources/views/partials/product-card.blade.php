...
<div class="group flex flex-col overflow-hidden rounded-xl bg-white shadow-md transition-all duration-300 hover:-translate-y-1 hover:shadow-xl">
    <div class="relative aspect-[3/2] overflow-hidden bg-cream">
        ...
        @if ($product->is_featured)
            <span class="absolute top-3 right-3 rounded-full bg-accent px-2.5 py-1 text-[10px] font-bold uppercase tracking-wide text-white shadow">Unggulan</span>
        @endif
    </div>
    ...
    <div class="flex flex-1 flex-col p-5">
        ...
        <h3 class="text-lg font-bold text-[#1A1A1A]">{{ $product->name }}</h3>
        <p class="mt-2 text-sm text-gray-600 line-clamp-3">{{ $product->short_description }}</p>

        @if (!empty($product->specifications))
            <dl class="mt-4 space-y-1 border-t border-gray-100 pt-3 text-xs text-gray-500">
                @foreach (array_slice($product->specifications, 0, 2, true) as $key => $value)
